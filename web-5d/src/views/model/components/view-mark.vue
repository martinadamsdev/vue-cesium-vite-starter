<template>
  <el-row :class="['view-tag-box', isShrink ? 'view-tag-box-shrink' : 'view-tag-box-spread']">
    <div :class="['tag-list', isShrink ? 'tag-list-shrink' : 'tag-list-spread']">
      <div class="nav-tab">
        <p class="tabs" :class="[active ? 'tab-active' : '']" @click="toggleMark(true)">我的标注</p>
        <p class="tabs" :class="[active ?  '' : 'tab-active']" @click="toggleMark(false)">共享标注</p>
        <el-checkbox v-model="checked" @change="handleActive">显示/隐藏</el-checkbox>
      </div>
      <el-table
        :data="tagList"
      >
  ...
  methods: {
    handleCurrentChange(val) {
      ...
      if (this.active) {
        ...
        this.queryShareMark(this.queryShareData)
      }
    },
    handleEdit(row) {
      this.$emit('handleEditMark', JSON.parse(JSON.stringify(row)))
    },
    handleDel(row) {
      this.$confirm('确认删除所选标注吗?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        loading('正在删除...')
        modelApi.delMark([row.id]).then(res => {
          loadingClose()
          this.$message({
            type: 'success',
            message: '删除成功'
    ...
    queryMark() {
      loading('数据加载中...')
      modelApi.getMark(this.queryData).then(res => {
        loadingClose()
    ...
    queryShareMark(parmas) {
      loading('数据加载中...')
      modelApi.getShareMark(parmas).then(res => {
        loadingClose()
        this.$set(this, 'tagList', res.list)
        this.total = res.total
      }).catch(error => {
